...
interface ValidatedRosterInput {
  name: string;
  dateRangeStart: string;
  dateRangeEnd: string;
  groupId: string;
...
function isRosterValidationError(
  result: RosterValidationError | ValidatedRosterInput
): result is RosterValidationError {
  return 'status' in result;
}

// Shared by POST / and PUT /:id: validates the roster+shifts payload (required fields, date
// range sanity, hoursPerShift, and ownership of the referenced group and shift templates) and
// returns either a validation error (with the same status/message the two handlers used to
// return inline) or the validated input ready to persist.
async function validateRosterInput(
  body: RosterInputBody,
  userId: string | undefined
): Promise<RosterValidationError | ValidatedRosterInput> {
  const { name, dateRangeStart, dateRangeEnd, groupId, shifts, hoursPerShift } = body;

  if (!name || !dateRangeStart || !dateRangeEnd || !groupId) {
    return { status: 400, error: 'name, dateRangeStart, dateRangeEnd and groupId are required' };
  }
  if (!shifts || shifts.length === 0) {
    return { status: 400, error: 'At least one shift is required' };
  }
  if (hoursPerShift !== undefined && hoursPerShift <= 0) {
    return { status: 400, error: 'hoursPerShift must be greater than 0' };
  }
  const rangeStart = new Date(dateRangeStart);
  const rangeEnd = new Date(dateRangeEnd);
  if (rangeStart > rangeEnd) {
    return { status: 400, error: 'dateRangeStart must not be after dateRangeEnd' };
  }
  for (const shift of shifts) {
    for (const date of shift.dates ?? []) {
      const d = new Date(date);
      if (d < rangeStart || d > rangeEnd) {
        return { status: 400, error: `Date ${date} is outside the roster's date range` };
      }
    }
  }
  ...
  const ownedTemplateIds = new Set(templates.filter((t) => t.userId === userId).map((t) => t.id));

  for (const shift of shifts) {
    if (!ownedTemplateIds.has(shift.shiftTemplateId)) {
      return { status: 404, error: `Shift template ${shift.shiftTemplateId} not found` };
    }
    if (!shift.dates || shift.dates.length === 0) {
      return { status: 400, error: 'Each shift needs at least one date' };
    }
    if (!shift.requirements || shift.requirements.length === 0) {
      return { status: 400, error: 'Each shift needs at least one responsibility requirement' };
    }
    for (const requirement of shift.requirements) {
      if (!requirement.responsibilityId) {
        return { status: 400, error: 'responsibilityId is required for each requirement' };
      }
      if (!requirement.headcount || requirement.headcount < 1) {
        return { status: 400, error: 'headcount must be at least 1' };
      }
    }
  }

  return { name, dateRangeStart, dateRangeEnd, groupId, shifts, hoursPerShift };
}
...
rosterRouter.get(
  ...
  withNotFoundHandling(async (req: AuthedRequest, res) => {
    const roster = await findOwnedOrThrow(
      () =>
...
rosterRouter.post('/', async (req: AuthedRequest, res) => {
  const validation = await validateRosterInput(req.body as RosterInputBody, req.userId);
  if (isRosterValidationError(validation)) {
    return res.status(validation.status).json({ error: validation.error });
  }
  const { name, dateRangeStart, dateRangeEnd, groupId, shifts, hoursPerShift } = validation;
...
rosterRouter.put(
  ...
  withNotFoundHandling(async (req: AuthedRequest, res) => {
    const existing = await findOwnedOrThrow(
      () => prisma.roster.findUnique({ where: { id: req.params.id } }),
      req.userId,
      'Roster not found'
    );
